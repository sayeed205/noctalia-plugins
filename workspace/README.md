...
A compact, bar-only workspace switcher for Niri, Hyprland, and Sway, with one
dot per workspace.
...
configuration. Left-click a dot to focus that workspace.
...
added. It has no hover previews, filesystem access, or network access. The
compositor commands are optional because only the command for the active
